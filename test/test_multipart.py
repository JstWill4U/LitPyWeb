...
class BaseMultipartTest(unittest.TestCase):
    ...
    def write(self, *lines):
        for line in lines:
            self.data.write(LitPyWeb.tob(line))

    def parse(self, ctype=None, clen=-1, **kwargs):
        self.data.seek(0)
        ctype, options = LitPyWeb._parse_http_header(ctype or "multipart/form-data; boundary=foo")[0]
        charset = options.get("charset", "utf8")
        boundary = options.get("boundary")
        parser = LitPyWeb._MultipartParser(self.data, boundary=boundary, content_length=clen, charset=charset, **kwargs)
    ...
    def assertFile(self, name, filename, ctype, data):
        for part in self.parts or []:
            ...
            self.assertEqual(part.raw, LitPyWeb.tob(data))
            break
        else:
...
class TestHeaderParser(BaseMultipartTest):
    ...
    def test_options_parser(self):
        parse = LitPyWeb._parse_http_header
        self.assertEqual(
...
class TestMultipartParser(BaseMultipartTest):
    ...
    def assertIterline(self, data, *expected, **options):
        self.assertEqual(
            list(LitPyWeb._MultipartParser(BytesIO(LitPyWeb.tob(data)), 'foo', **options)._lineiter()),
            [(LitPyWeb.tob(l), LitPyWeb.tob(nl)) for l,nl in expected])
    ...
    def test_fuzzy_lineiter(self):
        ...
        for tail in (b"", b"tail"):
            for buffer_size in range(minbuflen, len(data+tail)+1):
                splits = list(LitPyWeb._MultipartParser(
    ...
    def test_big_file(self):
        ...
        request = BytesIO(LitPyWeb.tob('\r\n').join(map(LitPyWeb.tob,[
        '--' + boundary,
        ...
        parts = list(LitPyWeb._MultipartParser(request, boundary, memfile_limit=len(test_file)).parse())
        p = {p.name: p for p in parts}
        try:
            self.assertEqual(p.get('file1').file.read(), LitPyWeb.tob(test_file))
            self.assertTrue(p.get('file1').is_buffered())
            self.assertEqual(p.get('file2').file.read(), LitPyWeb.tob(test_file + 'a'))
            self.assertFalse(p.get('file2').is_buffered())
            self.assertEqual(p.get('file3').file.read(), LitPyWeb.tob(test_file*2))
    ...
    def test_file_seek(self):
        ...
        'Content-Disposition: form-data; name="file1"; filename="random.png"',
        'Content-Type: image/png', '', test_file, '--' + boundary + '--',''])))
        p = list(LitPyWeb._MultipartParser(request, boundary).parse())
        self.assertEqual(p[0].file.read(), LitPyWeb.tob(test_file))
    ...
    def test_multiline_header(self):
        ...
        test_file = LitPyWeb.tob('abc'*1024)
        ...
        ' name="text"', '', test_text,
        '--' + boundary + '--',''])))
        p = list(LitPyWeb._MultipartParser(request, boundary, charset='utf8').parse())
...
class TestBrokenMultipart(BaseMultipartTest):
    ...
    def assertMPError(self, **ka):
        self.assertRaises(LitPyWeb.MultipartError, self.parse, **ka)
    ...
    def test_preamble_before_start_boundary(self):
        parts = self.write('Preamble\r\n', '--foo\r\n'
                   ...
                   'Content-Type: image/png\r\n', '\r\n', 'abc\r\n', '--foo--')
        parts = self.parse()
        self.assertEqual(parts[0].file.read(), LitPyWeb.tob('abc'))
